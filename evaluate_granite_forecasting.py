...
def main():
    ...
    parser.add_argument("--output_summary_csv", type=str, default=None,
                        help="Path to save aggregated summary (dataset × model), "
                             "e.g. granite_vs_autoreg_forecasting_summary.csv")
    ...
    parser.add_argument(
        ...
        help="Comma-separated models to evaluate: granite_ttm, autoreg, chronos",
    )
    parser.add_argument("--order", type=int, default=20, help="AR order for autoreg baseline")
    parser.add_argument("--num_samples", type=int, default=20, help="Number of forecast samples for Chronos")
    ...
    detectors = {
        model_name: build_detector(model_name, model_params)
    ...
    for dataset_name in dataset_names:
        ...
        for model_name, detector in detectors.items():
            for csv_path in iter_csv_files(dataset_root):
                rows = evaluate_file(csv_path, detector, model_name, model_params)
    ...
    if args.output_summary_csv:
        ...
        print(f"Saved summary metrics to:    {args.output_summary_csv}")
